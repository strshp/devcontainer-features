...
`~/.claude` 配下を、次の **2 種類** に分けて保存します。

| 種類 | 対象 | 保存先 |
|---|---|---|
| **ホスト共有**（認証情報＋設定） | `.credentials.json`（OAuth トークン）, `~/.claude.json`, `settings.json`, `settings.local.json`, `CLAUDE.md`, `keybindings.json`, `skills/`, `commands/`, `agents/`, `output-styles/`, `rules/`, `workflows/`, `themes/`, `plugins/` | ホストの `~/.claude`（＋ `~/.claude.json`）を bind マウントしてミラー |
...
- **コンテナ内で `sudo` がパスワードなしで使えること**（標準 devcontainer イメージは満たす）。
